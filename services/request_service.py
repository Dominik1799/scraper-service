...
def get_url_data(url: str) -> UrlDataResponse:
    ...
    if response == None:
        return UrlDataResponse(data="", downstream_response=0, got_reponse=False)
    return UrlDataResponse(data=response.text, downstream_response=response.status_code, got_reponse=True)