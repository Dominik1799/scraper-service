...
async def async_proxy_request(url: str, req_method: str, try_normal_request_first=True, timeout=10, headers={}, **kwargs):
    ua = UserAgent(
        browsers=BROWSERS,
        os=["windows", "linux", "macos"],
        platforms=["pc"]
    ...
    if try_normal_request_first:
        ...
        try:
            async with httpx.AsyncClient() as client:
                response = await client.request(method=req_method, url=url, timeout=timeout, headers=headers, **kwargs)
                if response.status_code >= 200 and response.status_code < 300:
                    return response
        except Exception:
            logger.debug("Normal request not working for " + url)
